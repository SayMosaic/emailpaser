...
Email.prototype = {
  ...
  read: function(text) {
    ...
    // }
    const patt = /^(On\s(\n|.)*wrote:)$/m;
    const doubleOnPatt = /^(On\s(\n|.)*(^(> )?On\s)((\n|.)*)wrote:)$/m;

    if(patt.test(text) && !doubleOnPatt.test(text)) {
      const reply_header = (patt.exec(text))[0];
    ...
    this.fragment = null;
      ...
      const lineList = lines.filter(i => i.length > 1);
      /* preprocessing email text like the following email text style
      email text
      ...
      .....
      in this case, regex can detect signature more trackable
      */
      const newlineList = [];
      ...
      for(let i in newlineList) {
        this.scan_line(newlineList[i]);
      }
  ...
  scan_line: function(line) {
    const SIG_REGEX = '(^\\*|^==|^--|__|\\w-$|\\d*\\|\\w*|\\w*\\|\\d+|\\w+\\|\\w+\\|\\w+)|(^(\\w+\\s*){0,3} ' + ("Sent from my").reverse() +  '\\s*$)';
    ...
    // quoted Fragment.
    const is_quoted = (new RegExp('(>+)$').test(line));
...
// signature.
const Fragment = function(quoted, first_line) {
