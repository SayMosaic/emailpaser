...
const contractions = require("./contractions_backup");

function deContractions(str) {
	return contractions.expand(str);
}
...
Email.prototype = {
  ...
  // Splits the given text into a list of Fragments.  This is roughly done by
	...
	scan_line: function(line) {
		let SIG_REGEX = '(^\\*|^==|^--|__|\\w-$|\\d*\\|\\w*|\\w*\\|\\d+|\\w+\\|\\w+\\|\\w+)|(^(\\w+\\s*){0,3} ' + ("Sent from my").reverse() +  '\\s*$)';
